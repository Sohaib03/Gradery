...
router
  .route("/create")
  .get(auth.authMiddleware, user_middleware.isInstructor, async (req, res) => {
...
router
  .route("/create")
  .post(auth.authMiddleware, user_middleware.isInstructor, async (req, res) => {
...
router.route("/code/:code").get(auth.authMiddleware, async (req, res) => {
  ...
  if (req.session.notification) context.notification = req.session.notification;
  res.render("teamHome", context);
});

router
  .route("/code/:code/notify")
  .post(user_middleware.isInstructor, auth.authMiddleware, async (req, res) => {
    const team_code = req.params.code;
    ...
    if (notif_title.trim().length === 0 || notif_content.trim().length === 0) {
