...
router
	...
	.get(auth.authMiddleware, async (req, res) => {
		...
		res.render("editAssignment", context);
	});
...
router
	...
	.post(auth.authMiddleware, async (req, res) => {
        ...
        await assignmentDB.updateAssignment(ass_id, title, desc);
        if (deadline_date) {
            await assignmentDB.setAssignmentDeadLine(ass_id, d_date + " " + d_time);
        }
        if (file) {
            await assignmentDB.setAssignmentFile(ass_id, file_name);
        }
        console.log({ass_id, title, desc, deadline_date, file_name});
        res.redirect("/assignment/" + ass_id);
